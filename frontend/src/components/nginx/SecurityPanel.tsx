...
const SecurityPanel: React.FC = () => {
  return (
    <div>
      ...
      <Form.Item noStyle shouldUpdate={(prev, cur) => prev.rate_limit_enabled !== cur.rate_limit_enabled}>
        {({ getFieldValue }) =>
          getFieldValue('rate_limit_enabled') && (
            <Row gutter={16}>
              <Col span={16}>
                <Form.Item label="限速规则" name="rate_limit_zone" extra="例如：$binary_remote_addr zone=one:10m rate=10r/s">
                  <Input placeholder="$binary_remote_addr zone=one:10m rate=10r/s" style={{ fontFamily: 'var(--font-mono)', fontSize: 12 }} />
                </Form.Item>
              </Col>
              <Col span={8}>
                <Form.Item label="突发请求数" name="rate_limit_burst">
      ...
      <Form.Item noStyle shouldUpdate={(prev, cur) => prev.conn_limit_enabled !== cur.conn_limit_enabled}>
        {({ getFieldValue }) =>
          getFieldValue('conn_limit_enabled') && (
            <Row gutter={16}>
              <Col span={16}>
                <Form.Item label="连接限制规则" name="conn_limit_zone" extra="例如：$binary_remote_addr zone=addr:10m">
      ...
      <Row gutter={16}>
        <Col span={12}>
          <Form.Item label="允许访问的 IP" name="allow_ips" extra="每行填写一个 IP 或 CIDR">
            <TextArea rows={3} placeholder="192.168.1.0/24&#10;10.0.0.0/8" style={{ fontFamily: 'var(--font-mono)', fontSize: 12 }} />
          </Form.Item>
        </Col>
        <Col span={12}>
          <Form.Item label="拒绝访问的 IP" name="deny_ips" extra="每行填写一个 IP 或 CIDR">
      ...
      <Form.Item label="安全响应头（JSON）" name="security_headers" extra='例如：{"X-Frame-Options": "SAMEORIGIN", "X-Content-Type-Options": "nosniff"}'>
        <TextArea
          rows={3}
